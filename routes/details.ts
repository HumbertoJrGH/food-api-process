...
export default async function Details(req: Request, res: Response) {
	try {
		...
		const info = await page.evaluate(async () => {
			...
			if (score) {
				...
				if (listValues)
					listValues.forEach(async item => {
						const img: HTMLImageElement | null = item.querySelector("a img")
						values.push([
			...
			const img: HTMLImageElement | null = document.querySelector("#panel_nova li a img")
			const nova = {
